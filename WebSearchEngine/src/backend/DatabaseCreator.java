...
public class DatabaseCreator {
	...
	public void create() {
		try {
			...
			System.out.println("createProcedureToTfIdfUpdate");
			createFunctionConjunctive_search(connection);
			System.out.println("createFunctionConjunctive_search");
			createFunctionDisjunctive_search(connection);			
			System.out.println("createFunctionDisjunctive_search");
	...
	private void createFeatureTable(Connection con) throws SQLException {
		PreparedStatement statement = con.prepareStatement(
				"CREATE TABLE IF NOT EXISTS   features (docid INT REFERENCES documents(docid), term TEXT, term_frequency INT)");
		statement.execute();
		statement.close();
	}
	...
	private void createDocumentsTable(Connection con) throws SQLException {
		PreparedStatement statement = con.prepareStatement(
				"CREATE TABLE IF NOT EXISTS  documents (docid INT PRIMARY KEY , url TEXT NOT NULL , crawled_on_date DATE, language TEXT)");
		statement.execute();
		statement.close();
	}
